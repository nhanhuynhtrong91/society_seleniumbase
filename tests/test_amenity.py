...
#             self.assert_equal(int(val_C_page2), C)
    ...
    def test_login_with_owner_2(self):
        ...
        owner_page = AmenityPage(self.driver)
        print(owner_page.page_url)
        self.assert_url_contains("amenities")
        owner_page.getFilters()
        owner_page.getOptionsByFilter("filterBookingRequired")
        valueToFilter = "Yes"
        if owner_page.selectOption(valueToFilter):
            results = self.find_elements("table tbody tr")
            matchRows = 0
            for element in results:
                element = self.driver.find_element(By.XPATH, '//td[normalize-space(text())="Yes"]')
                if element and element.text == valueToFilter:
                    matchRows += 1
